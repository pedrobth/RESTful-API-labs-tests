...
describe('INSERT laboratory tests associations', () => {
  ...
  it('check a single valid associations can be inserted', async () => {
    const postResponse = await frisby.post(URL_LAB_3, [{
  ...
  it('application handle invalid association -> invalid laboratory name', async () => {
    const postResponse = await frisby.post(`${URL_LAB_3} InvalidLabName`, [{ testName: TESTS[0].testName }])
      .expect('status', 400);
    const { body } = postResponse;
    const parsedResponse = JSON.parse(body);
  ...
  it('application handle invalid association -> invalid test name', async () => {
    const postResponse = await frisby.post(URL_LAB_3, [{ testName: `${TESTS[0].testName} InvalidTestName` }])
      .expect('status', 400);
    const { body } = postResponse;
    const parsedResponse = JSON.parse(body);
    expect(parsedResponse.message).toBe('associations between inactive or out of database laboratory or between valid laboratory and inactive or out of databse test is forbiden. check your inputs');
  ...
  it('SQL injection test in test field', async () => {
    const postResponse = await frisby.post(URL_LAB_2, [{ testName: `${TESTS[3].testName}; DROP TABLE tests_laboratories`}])
      .expect('status', 400);
    const { body } = postResponse;
    const parsedResponse = JSON.parse(body);
    expect(parsedResponse.message).toBe('associations between inactive or out of database laboratory or between valid laboratory and inactive or out of databse test is forbiden. check your inputs');
  ...
  it('SQL injection test in test field', async () => {
    const postResponse = await frisby.post(`${URL_LAB_2}; DROP TABLE tests_laboratories`, [{ testName: TESTS[3].testName}])
      .expect('status', 400);
    const { body } = postResponse;
    const parsedResponse = JSON.parse(body);
    expect(parsedResponse.message).toBe('associations between inactive or out of database laboratory or between valid laboratory and inactive or out of databse test is forbiden. check your inputs');
